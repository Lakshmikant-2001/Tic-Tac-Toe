...
const aiRandomLvl = document.querySelector(".level-options > :first-child");
...
export const removeCurrentLevel = function () {
  aiLvlOptions.forEach((option) => {
    option.classList.remove("current-level");
  })
}

export const clearAiLvl = function () {
  removeCurrentLevel();
  aiRandomLvl.classList.add("current-level");
}
